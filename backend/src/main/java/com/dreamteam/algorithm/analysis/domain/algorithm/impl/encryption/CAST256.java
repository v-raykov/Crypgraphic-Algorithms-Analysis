...
@Component
public class CAST256 implements EncryptionAlgorithm, RequiresIv, MultipleFixedKeySizes {
    ...
    private byte[] processData(boolean encrypt, byte[] data, byte[] key) throws Exception {
        if (!isValidKey(key)) {
            throw new IllegalArgumentException("Invalid key size for CAST-256");
        }
    ...
    public void setIv(byte[] iv) {
        if (iv == null || iv.length != ivSize) {
            throw new IllegalArgumentException("Invalid IV size for CAST-256");
        }
        this.iv = iv;
    }
